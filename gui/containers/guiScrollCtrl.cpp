...
ConsoleMethod(GuiScrollCtrl, scrollToObject, void, 3, 3, "(obj) - scrolls the scroll control to view the specified object.")
{
	GuiControl* targetControl = dynamic_cast<GuiControl*>(Sim::findObject(argv[2]));
	if (targetControl)
	{
		object->scrollToObject(targetControl);
	}
	else
		Con::errorf("GuiScrollCtrl::scrollToObject() - Invalid control specified (%s)!", argv[2]);
}
...
ConsoleMethod(GuiScrollCtrl, computeSizes, void, 2, 2, "() - Refresh the sizes of the child objects.")
{
   object->computeSizes();
}
...
void GuiScrollCtrl::initPersistFields()
{
   ...
   addField("hScrollBar",           TypeEnum,    Offset(mForceHScrollBar, GuiScrollCtrl), 1, &gScrollBarTable, "When to display the horizontal scrollbar.");
   addField("vScrollBar",           TypeEnum,    Offset(mForceVScrollBar, GuiScrollCtrl), 1, &gScrollBarTable, "When to display the vertical scrollbar.");
   addField("lockHorizScroll",      TypeBool,    Offset(mLockHorizScroll, GuiScrollCtrl), "Horizontal scrolling not allowed if set.");
   addField("lockVertScroll",       TypeBool,    Offset(mLockVertScroll, GuiScrollCtrl), "Vertical scrolling not allowed if set.");
   ...
   addField("mouseWheelScrollSpeed", TypeS32,    Offset(mScrollAnimSpeed, GuiScrollCtrl), "Pixels/Tick - if not positive then mousewheel scrolling occurs instantly (like other scrolling).");
...
bool GuiScrollCtrl::calcChildExtents()
{
   ...
   // Get the greatest position and extent points so the scroll bars will function function
	// correctly with multiple gui controls. We are grabbing the current values of ext and pos
	// to utilize later.
	Point2I extApex = mChildExt;
	Point2I posApex = mChildPos;

	// This will be our check in order to fail out if we need to
   ...
   for ( U32 i = 0; i < size(); i++ )
   {
      ...
      if ( ctrl->isVisible() )
      {
			// Always grab the initial values
			if ( !hasChildren )
			{
				extApex = ctrl->getExtent();
				posApex = ctrl->getPosition();
				hasChildren = true;
			}
			else
			{
				// Now we can do continuous checks on those values
				if( extApex.x < ctrl->getExtent().x )
					extApex.x = ctrl->getExtent().x;

				if( extApex.y < ctrl->getExtent().y )
					extApex.y = ctrl->getExtent().y;
				
				if( posApex.x < ctrl->getPosition().x )
					posApex.x = ctrl->getPosition().x;

				if( posApex.y < ctrl->getPosition().y )
					posApex.y = ctrl->getPosition().y;
			}
      }
   }
	...
	if( hasChildren )
	{
		mChildExt.x = extApex.x;
		mChildExt.y = extApex.y;

		mChildPos.x = posApex.x;
		mChildPos.y = posApex.y;
...
bool GuiScrollCtrl::isRectCompletelyVisible(RectI rect)
{
   // rect is passed in virtual client space
   // Determine the points bounding the requested rectangle
	Point2I rectUpperLeft  = rect.point;
