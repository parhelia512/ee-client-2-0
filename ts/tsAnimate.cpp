...
void TSShapeInstance::animateNodes(S32 ss)
{
   ...
   if (mShape->nodes.size() > mNodeTransforms.size())
   ...
   // @todo: Need to update TSShapeInstances when the number of nodes changes.....
   mNodeTransforms.setSize(smNodeCurrentRotations.size());
...
void TSShapeInstance::animateIfls()
{
   ...
   for (S32 i=0; i<mIflMaterialInstances.size(); i++)
   {
      ...
      for (S32 j=0; j<mThreadList.size(); j++)
      {
         ...
         if (th->getSequence()->iflMatters.test(i))
         {
            ...
            F32 time = th->pos * th->getSequence()->duration + th->getSequence()->toolBegin;
