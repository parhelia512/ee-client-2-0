...
void TSMesh::innerRender( TSMaterialList *materials, const TSRenderState &rdata, TSVertexBufferHandle &vb, GFXPrimitiveBufferHandle &pb )
{
   ...
   SceneState *state = rdata.getSceneState();
   ...
   // Let the light manager fill the RIs light
   // vector with the current best lights.
   gClientSceneGraph->getLightManager()->getBestLights( coreRI->lights, 8 );

   coreRI->visibility = meshVisibility;  
   coreRI->translucentSort = meshVisibility < VISIBILITY_EPSILON;
   ...
   for ( S32 i = 0; i < primitives.size(); i++ )
   {
      ...
      const bool isMatIndexValid = materials->getMaterialCount() > matIndex;
      ...
      *ri = *coreRI;
      ...
      if ( matInst )
      {
         BaseMaterialDefinition *mat = matInst->getMaterial();

         ri->matInst = matInst;
         ri->defaultKey = (U32)mat; // This is not 64bit safe!
         ri->primBuffIndex = i;

         // Use the material to figure out if we need translucent sorting.
         ri->translucentSort = mat->isTranslucent();

         // may need to force translucent on visibility < 1.0
         if (coreRI->visibility < 1.0f && !coreRI->translucentSort)
         {
           // first check for refraction, don't want translucent in that case
           CustomMaterial* cust = dynamic_cast<CustomMaterial*>(matInst->getMaterial());
           if (!cust || !cust->mRefract)
             ri->translucentSort = true;
         }

         // if translucent then we may need to switch type in order to get
         // rendered by the correct render manager
         if (ri->translucentSort && ri->type == RenderPassManager::RIT_Mesh)
           ri->type = RenderPassManager::RIT_Translucent;

         // fill in misc texture if IFL - need it to override base texture since it changes frequently
         if ( mat->isIFL() && isMatIndexValid )
            ri->miscTex = materials->getMaterial( matIndex ).getPointer();
      }
      else
      {
         ...
         // Without a material we assume the mesh doesn't require
         // translucent sorting before rendering.
         ...
         U32 flags = 0;
         if ( isMatIndexValid )
            flags = materials->getFlags( matIndex );
         ...
         ri->defaultKey = ri->miscTex ? (U32) ri->miscTex : (U32) ri->matInst;
      }
...
void TSSkinMesh::createBatchData()
{
   ...
   for(int i = 0; i < numBatchOps; i++)
   {
      ...
      curTransform.alignedMem = reinterpret_cast<BatchData::BatchedVertWeight *>(dAligned_malloc(sizeof(BatchData::BatchedVertWeight) * numVerts, 16));
...
void TSMesh::_convertToAlignedMeshData( TSMeshVertexArray &vertexData, const Vector<Point3F> &_verts, const Vector<Point3F> &_norms )
{
   ...
   if(mVertexData.isReady() && vertexData.address() != mVertexData.address())
   {
      ...
      void *aligned_mem = dAligned_malloc(mVertSize * mNumVerts, 16);
