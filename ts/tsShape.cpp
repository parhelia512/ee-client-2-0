...
TSShape::TSShape()
{
   ...
   VECTOR_SET_ASSOCIATION(iflFrameOffTimes);
   ...
   VECTOR_SET_ASSOCIATION( iflMaterials );
...
{
   if(materialList)
      return materialList->load(path.getPath());
...
void TSShape::disassembleShape()
{
   ...
   S32 numIflMaterials = tsalloc.set32(iflMaterials.size());
   ...
   tsalloc.setGuard();
   tsalloc.copyToBuffer32((S32*)iflMaterials.address(),numIflMaterials*5);
   tsalloc.setGuard();
...
{
   // Determine whether we can write this shape in TSTPRO compatible
   // format
   bool saveOldFormat = true;

   for (S32 i = 0; i < meshes.size() && saveOldFormat; i++)
   {
      if (!meshes[i])
         continue;
   ...
   // HACK: Disabled for now till we work out how to save 
   // the new autobillboard details and if we still want
   // to support ShowTool Pro.
   saveOldFormat = false;
...
bool TSShape::read(Stream * s)
{
   ...
   S32 buffSize = tsalloc.getSize();
   tsalloc.doAlloc();
   mShapeData = tsalloc.getBuffer();
   tsalloc.setRead(memBuffer32,memBuffer16,memBuffer8,false);
   assembleShape(); // copy to buffer
   AssertFatal(tsalloc.getSize()==buffSize,"TSShape::read: shape data buffer size mis-calculated");
...
void TSShape::readIflMaterials(const char* shapePath)
{
   ...
   for (S32 i=0; i<iflMaterials.size(); i++)
   {
      ...
      String pName = getName(iflMaterial.nameIndex);

      // Open the file which should be located in the same directory
      // as the .dts shape.
      String   fileName = String::ToString("%s/%s", shapePath, pName.c_str());

      FileStream  *fs = FileStream::createAndOpen( fileName, Torque::FS::File::Read );
      ...
      String materialName = MATMGR->getMapEntry(pName);
      if(materialName.isNotEmpty())
         matInst = MATMGR->createMatInstance(materialName);
      ...
      while (fs->getStatus() == Stream::Ok)
      {
         ...
         if (fs->getStatus() == Stream::Ok || fs->getStatus() == Stream::EOS)
         {
            ...
            // set material instance if one is there
            if( matInst )
            {
...
template<> void *Resource<TSShape>::create(const Torque::Path &path)
{
   ...
   {
      FileObject script;
      if (script.readMemory(scriptPath.getFullPath()))
      {
         String instantGroup = Con::getVariable("InstantGroup");
         Con::setIntVariable("InstantGroup", RootGroupId);
         Con::evaluate((const char*)script.buffer(), false, scriptPath.getFullPath());
