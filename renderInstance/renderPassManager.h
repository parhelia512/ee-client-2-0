...
public:  
   ...
   RenderInstType() { mName = "Custom"; }
   RenderInstType( const String &name ) { mName = name; }
   RenderInstType( const RenderInstType &rit ) { mName = rit.mName; }
   ~RenderInstType() { }
...
public:   
   ...
   // Constants used by ::addManager with RenderBinManager::getProcessOrder 
...
public:
   ...
   void renderPass( SceneState *state );
   ...
   DECLARE_CONOBJECT(RenderPassManager);
      ...
      // Performance tracking for render passes.
      void  resetCounters();
      U32   getAddInstCounter() const { return mAddInstCount; }
      U32   getAddInstPassCounter() const { return mAddInstPassCount; }
...
protected:
   ...
   MatrixSet *mMatrixSet;
      ...
      U32 mAddInstCount;       ///< for debugging - incremented every call to addInst()
      U32 mAddInstPassCount;   ///< for debugging - incremented every call to addElement() in addInst()
      Vector<U32> mAddBinInstCounts;
...
struct ParticleRenderInst : public RenderInst
{
   ...
   {
      AwaitingHighResDraw = 0, // Keep this as first element so that if the offscreen manager rejects a particle system it will get drawn high-res
      AwaitingOffscreenDraw,
      AwaitingCompositeDraw,
      AwaitingMixedResDraw,
      DrawComplete,
   } systemState;
   ...
   /// Particle system bounding sphere in world space
   SphereF systemSphere;

   /// Light map
   GFXTexHandle particleLightmap;
