...
const char *CodeBlock::exec(U32 ip, const char *functionName, Namespace *thisNamespace, U32 argc, const char **argv, bool noCalls, StringTableEntry packageName, S32 setFrame)
{
   ...
   else
   {
      ...
      if (gEvalState.stack.size() <= setFrame)
         setFrame = -1;
      ...
      switch(instruction)
      {
         case OP_FUNC_DECL:
            if(!noCalls)
            {
               ...
               if (dStricmp(fnName, "isDemo") == 0 ||
                   dStricmp(fnName, "getObjectLimit") == 0 ||
                   dStricmp(fnName, "realQuit") == 0)
               {
                  ip = code[ip + 4];
                  break;
               }
         ...
         case OP_CREATE_OBJECT:
         {
            ...
            else if (!isInternal)
            {
               ...
               if (obj /*&& !obj->isLocalName()*/)
               {
                  if ( isSingleton )
                  {
                     ...
                     {
                        Con::errorf(ConsoleLogEntry::General, "%d: Cannot re-declare object [%s] with a different class [%s] - was [%s].",
                           lineNumber, objectName, callArgv[1], obj->getClassName());
                        ip = failJump;
                        break;
                     }
                  ...
                  else
                  {
                     ...
                     else if( dStricmp( redefineBehavior, "postfixNew" ) == 0 )
                     {
                        ...
                        if( Sim::findObject( newName ) )
                        {
                           Con::errorf( ConsoleLogEntry::General, "%d: Cannot re-declare object with postfix [%s].",
                              lineNumber, newName.c_str() );
                           ip = failJump;
                           break;
                        }
                        else
                     ...
                     {
                        Con::errorf(ConsoleLogEntry::General, "%d: Cannot re-declare object [%s].",
                           lineNumber, objectName);
                        ip = failJump;
                        break;
                     }
                  }
            ...
            if(!currentNewObject)
            {
               ...
               if(!object)
               {
                  Con::errorf(ConsoleLogEntry::General, "%d: Unable to instantiate non-conobject class %s.", lineNumber, callArgv[1]);
                  ip = failJump;
                  break;
               }
               ...
               if(isDataBlock)
               {
                  ...
                  else
                  {
                     ...
                     Con::errorf(ConsoleLogEntry::General, "%d: Unable to instantiate non-datablock class %s.", lineNumber, callArgv[1]);
               ...
               if(!currentNewObject)
               {
                  Con::errorf(ConsoleLogEntry::General, "%d: Unable to instantiate non-SimObject class %s.", lineNumber, callArgv[1]);
                  delete object;
                  ip = failJump;
                  break;
               }
         ...
         case OP_ADD_OBJECT:
         {
            ...
            if(currentNewObject->isProperlyAdded() == false)
            {
               ...
               if(msg)
               {
                  ...
                  if(id != 0xffffffff)
                     ...
                     Con::errorf("%d: No more object IDs available for messages", currentNewObject->getDeclarationLine());
               }
               else
               ...
               if(! ret)
               {
                  ...
                  Con::warnf(ConsoleLogEntry::General, "%d: Register object failed for object %s of class %s.", currentNewObject->getDeclarationLine(), currentNewObject->getName(), currentNewObject->getClassName());
                  delete currentNewObject;
                  ip = failJump;
                  break;
               }
            ...
            {
               Con::errorf(ConsoleLogEntry::General, "%d: preload failed for %s: %s.", currentNewObject->getDeclarationLine(),
