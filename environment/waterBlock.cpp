...
void WaterBlock::unpackUpdate(NetConnection* con, BitStream* stream)
{
   ...
   if( stream->readFlag() ) // UpdateMask
   {
      ...
      if ( isProperlyAdded() && scale != mObjScale )
         mGenerateVB = true;
      mObjScale = scale;
...
void WaterBlock::setupVertexBlock( U32 width, U32 height, U32 rowOffset )
{
   ...
   if ( lm )
   {
      LightInfo* linfo = lm->getSpecialLight( LightManager::slSunLightType );
      if ( linfo )
...
//-----------------------------------------------------------------------------
void WaterBlock::setShaderParams( SceneState *state, BaseMatInstance *mat, const WaterMatParams &paramHandles)
{
   ...
   matParams->set(paramHandles.mModelMatSC, modelMat, GFXSCT_Float3x3);
   ...
   //Point4F reflectParams( mPlaneReflector.position.z, mReflectMinDist, mReflectMaxDist, mPlaneReflector.enabled ? 0.0f : 1.0f );   
...
void WaterBlock::innerRender( SceneState *state )
{      
...
bool WaterBlock::setGridSizeProperty(void* obj, const char* data)
{
