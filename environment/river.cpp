...
void River::consoleInit()
{
   ...
   Con::addVariable( "$River::showWalls", TypeBool, &River::smShowWalls );
   Con::addVariable( "$River::showNodes", TypeBool, &River::smShowNodes );
   Con::addVariable( "$River::showSpline", TypeBool, &River::smShowSpline );
   Con::addVariable( "$River::showRiver", TypeBool, &River::smShowRiver );
	Con::addVariable( "$River::showWireframe", TypeBool, &River::smWireframe );
}

bool River::addNodeFromField( void* obj, const char* data )
{
   River *pObj = static_cast<River*>(obj);
...
void River::innerRender( SceneState *state )
{   
   ...
   while( mat->setupPass( state, sgData ) )
   {
      ...
      for ( U32 i = 0; i < mLowLODBatches.size(); i++ )
      {
         ...
         GFX->drawIndexedPrimitive( GFXTriangleList, 0, 0, vertCount, batch.startSegmentIdx * 6, triangleCount );
      }
      ...
      if ( mVB_high.isValid() && mVB_low.isValid() )
      {
...
SceneGraphData River::setupSceneGraphInfo( SceneState *state )
{
   ...
   // The river is in world space already... no transform!
...
void River::updateUnderwaterEffect( SceneState *state )
{
...
void River::setShaderParams( SceneState *state, BaseMatInstance* mat, const WaterMatParams& paramHandles )
{
   ...
   matParams->set(paramHandles.mGridElementSizeSC, 1.0f);
   matParams->set(paramHandles.mModelMatSC, MatrixF::Identity, GFXSCT_Float3x3);
   ...
   //Point4F reflectParams( mReflectPlaneWorldPosition.z, mReflectMinDist, mReflectMaxDist, mFullReflect ? 0.0f : 1.0f );
   Point4F reflectParams( mWaterPos.z, 0.0f, 1000.0f, mPlaneReflector.isEnabled() ? 0.0f : 1.0f );
   matParams->set(paramHandles.mReflectParamsSC, reflectParams );

   matParams->set(paramHandles.mReflectNormalSC, mPlaneReflector.refplane );   
}

bool River::isUnderwater( const Point3F &pnt )
{
   return containsPoint( pnt );
}
...
bool River::containsPoint( const Point3F &worldPos, U32 *nodeIdx )
{
...
void River::_makeHighLODBuffers()
{
   ...
   mVB_high.set( GFX, numVerts, GFXBufferTypeDynamic );
   ...
   mPB_high.set( GFX, numTriangles * 3, numTriangles, GFXBufferTypeDynamic );         
   U16 *idxBuff;
   ...
   AssertFatal( curIdx == numTriangles * 3, "River, wrote incorrect number of indicies to mPV_high" );
...
ConsoleMethod( River, regenerate, void, 2, 2, "setRegenFlag()" )
{
   object->regenerate();
}

ConsoleMethod( River, setMetersPerSegment, void, 3, 3, "setMetersPerSegment( F32 meters )" )
{
   object->setMetersPerSegment( dAtoi(argv[2]) );
}

ConsoleMethod( River, setBatchSize, void, 3, 3, "setBatchSize( U32 size )" )
{
   object->setBatchSize( dAtoi(argv[2]) );
}

ConsoleMethod( River, setNodeDepth, void, 4, 4, "setNodeDepth( U32 idx, F32 meters )" )
{
   object->setNodeDepth( dAtoi(argv[2]), dAtof(argv[3]) );
}

ConsoleMethod( River, setMaxDivisionSize, void, 3, 3, "setMaxDivisionSize( F32 meters )" )
{
   object->setMaxDivisionSize( dAtof(argv[2]) );
}
