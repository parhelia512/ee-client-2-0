...
void MeshRoad::initPersistFields()
{
   addGroup( "MeshRoad" );
      ...
      addField( "widthSubdivisions", TypeS32, Offset( mWidthSubdivisions, MeshRoad ), "Subdivide segments widthwise this many times when generating vertices." );
...
void MeshRoad::consoleInit()
{
   ...
   Con::addVariable( "$MeshRoad::EditorOpen", TypeBool, &MeshRoad::smEditorOpen );
   Con::addVariable( "$MeshRoad::wireframe", TypeBool, &MeshRoad::smWireframe );
   Con::addVariable( "$MeshRoad::showBatches", TypeBool, &MeshRoad::smShowBatches );
   Con::addVariable( "$MeshRoad::showSpline", TypeBool, &MeshRoad::smShowSpline );
   Con::addVariable( "$MeshRoad::showRoad", TypeBool, &MeshRoad::smShowRoad );
}

bool MeshRoad::addNodeFromField( void* obj, const char* data )
{
...
bool MeshRoad::prepRenderImage( SceneState* state, 
                              const U32   stateKey,
                              const U32, 
                              const bool )
{
   if ( mNodes.size() <= 1 || isLastState(state, stateKey) ) 
      return false;
...
void MeshRoad::_debugRender( ObjectRenderInst *ri, SceneState *state, BaseMatInstance* )
{
...
bool MeshRoad::buildPolyList(AbstractPolyList* polyList, const Box3F &box, const SphereF &)
{
   if ( mSlices.size() < 2 )
      return false;
...
ConsoleMethod( MeshRoad, regenerate, void, 2, 2, "setRegenFlag()" )
{
   object->regenerate();
}

ConsoleMethod( MeshRoad, setMetersPerSegment, void, 3, 3, "setMetersPerSegment( F32 meters )" )
{
   object->setMetersPerSegment( dAtoi(argv[2]) );
}

ConsoleMethod( MeshRoad, setNodeDepth, void, 4, 4, "setNodeDepth( U32 idx, F32 meters )" )
{
   object->setNodeDepth( dAtoi(argv[2]), dAtof(argv[3]) );
}

ConsoleMethod( MeshRoad, postApply, void, 2, 2, "")
{
