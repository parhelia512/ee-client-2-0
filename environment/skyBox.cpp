...
bool SkyBox::onAdd()
{
   ...
   if ( isClientObject() )
   {
      ...
      transparent.setZReadWrite( true, false );
      transparent.setBlend( true, GFXBlendSrcAlpha, GFXBlendInvSrcAlpha );
      mFogBandSB = GFX->createStateBlock( transparent );
...
bool SkyBox::prepRenderImage( SceneState *state, const U32 stateKey, const U32 startZone, const bool modifyBaseZoneState )
{
...
void SkyBox::_renderObject( ObjectRenderInst *ri, SceneState *state, BaseMatInstance *mi )
{
   ...
   if ( state->isReflectPass() )
      mMatrixSet->setProjection( gClientSceneGraph->getNonClipProjection() );
   ...
   {         
      ...
      if ( mFogBandHeight > 0 )
      {
         const FogData &fog = gClientSceneGraph->getFogData();
         ...
         MatrixF camMat = sgData.objTrans;
         camMat.setPosition( Point3F::Zero );
         camMat.inverse();

         GFX->setStateBlock( mFogBandSB );
         GFX->setWorldMatrix( camMat );
