...
void Interior::debugRender(const ZoneVisDeterminer& zoneVis, SceneGraphData &sgData, InteriorInstance *intInst, MatrixF& modelview)
{
	...
	if ( mDebugShader == NULL )
   {
      ...
      if ( mDebugShader )
      {
         ...
         shaderData->mapSamplerNames( mDebugShaderConsts );
   ...
   if (mDebugShaderModelViewSC->isValid())
      mDebugShaderConsts->set(mDebugShaderModelViewSC, modelview);
...
void Interior::debugShowSurfaceFlag(const ZoneVisDeterminer& zoneVis, const U32 flag, const ColorF& c)
{
	...
	for (U32 i = 0; i < mZones.size(); i++)
   {
      ...
      for( U32 j=0; j<mZones[i].surfaceCount; j++ )
      {
         ...
         if (mDebugShaderShadeColorSC->isValid())
            mDebugShaderConsts->set(mDebugShaderShadeColorSC, col);
...
void Interior::debugShowPortalZones(const ZoneVisDeterminer& zoneVis)
{
   ...
   for (U32 i = 0; i < mZones.size(); i++)
   {
      ...
      for (U32 j = mZones[i].surfaceStart; j < mZones[i].surfaceStart + mZones[i].surfaceCount; j++)
      {
         ...
         if (mDebugShaderShadeColorSC->isValid())
            mDebugShaderConsts->set(mDebugShaderShadeColorSC, c);
												 ...
												 InteriorInstance *intInst)
{
   ...
   for( U32 i=0; i<getNumZones(); i++ )
   {
      ...
      for( U32 j=0; j<mZoneRNList[i].renderNodeList.size(); j++ )
      {
         ...
         if (mDebugShaderShadeColorSC->isValid())
            mDebugShaderConsts->set(mDebugShaderShadeColorSC, texSizeColor);
