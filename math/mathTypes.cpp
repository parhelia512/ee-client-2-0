...
ConsoleFunction( VectorLerp, const char*, 4, 4, "(Vector3F a, Vector3F b, F32 t ) Return vector interpolated between a / b with time t." )
{
   VectorF a,b,c;
   dSscanf( argv[1], "%g %g %g", &a.x, &a.y, &a.z );
   dSscanf( argv[2], "%g %g %g", &b.x, &b.y, &b.z );

   c.interpolate( a, b, dAtof( argv[3] ) );
   char *ret = Con::getReturnBuffer(256);
   dSprintf( ret, 256, "%g %g %g", c.x, c.y, c.z );
