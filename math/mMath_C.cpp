...
bool m_planeF_intersect_box3F_C(const F32 *planes, const F32 *box)
{
   ...
   const Box3F &bounds = *reinterpret_cast<const Box3F *>(box);

   // Note the planes are ordered left, right, near, 
   // far, top, bottom for getting early rejections
   // from the typical horizontal scene.
   const PlaneF *plane = reinterpret_cast<const PlaneF *>(planes);
   const PlaneF *endPlane = plane + Frustum::PlaneCount;

   Point3F maxPoint;
   F32 maxDot;

   while( plane < endPlane )
   {
      const PlaneF &curPlane = *plane;
      ...
      maxPoint.x = ( curPlane.x > 0.0f ) ? bounds.maxExtents.x : bounds.minExtents.x;
      maxPoint.y = ( curPlane.y > 0.0f ) ? bounds.maxExtents.y : bounds.minExtents.y;
      maxPoint.z = ( curPlane.z > 0.0f ) ? bounds.maxExtents.z : bounds.minExtents.z;

      maxDot = mDot( maxPoint, curPlane );
...
bool (*m_planeF_intersect_box3F)(const F32 *planes, const F32 *bounds) = m_planeF_intersect_box3F_C;
...
//    m_matF_x_vectorF        = m_matF_x_vectorF_C;
   ...
   m_planeF_intersect_box3F = m_planeF_intersect_box3F_C;
}
