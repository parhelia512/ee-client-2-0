...
/// The buffer interface hides the details of how the device
/// holds sound data for playback.
///
/// A sound buffer may either be loaded once completely and then
/// played as needed or it may be progressively streamed from
/// an SFXStream.  In the latter case, there can only be a single
...
class SFXBuffer :    public StrongRefBase,
                     public IPolled,
                     public IOutputStream< SFXInternal::SFXStreamPacket* >
{
   ...
   protected:
      ...
      /// If "createAsyncStream" is true, the asynchronous loading state for the
      /// buffer will be set up in the constructor.
      ...
      void _setStatus( EStatus status );
   ...
   public:
      ...
      ///   code runs on.
      Signal< void( SFXBuffer* buffer, EStatus newStatus ) > mOnStatusChange;
