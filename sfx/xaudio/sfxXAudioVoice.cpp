...
void SFXXAudioVoice::_flush()
{
   ...
   // According to the docs, SamplesPlayed reported by the
   // voice should be zero now.  Alas it ain't.  So, save
   // the current value here and offset our future play
   // cursors.

   XAUDIO2_VOICE_STATE state;
   mXAudioVoice->GetState( &state );

   mSamplesPlayedOffset = state.SamplesPlayed;
...
void SFXXAudioVoice::_play()
{
   ...
   {
      AssertFatal( !mBuffer->isStreaming(), "SFXXAudioVoice::_play() - non data queued on streaming voice" );
      _loadNonStreamed();
   }
...
void SFXXAudioVoice::_seek( U32 sample )
{
   ...
   U32 pos = sample * format.getBytesPerSample();
   pos = mAlignToMultiple( pos, format.getChannels() * format.getBitsPerSample() / 8 );
   mNonStreamBufferOffset = pos;
...
void SFXXAudioVoice::_loadNonStreamed()
{
   ...
   if( _status() == SFXStatusPlaying )
      _stop(); // Will also flush buffers.
   ...
   mNonStreamBuffer.pAudioData = orgBuffer.pAudioData + mNonStreamBufferOffset;
   mNonStreamBuffer.AudioBytes = orgBuffer.AudioBytes - mNonStreamBufferOffset;
   if( mIsLooping )
...
U32 SFXXAudioVoice::_tell() const
{
   ...
   // Handle end-of-stream.  SamplesPlayed will
   // be zero and no buffers queued.

   if( !state.BuffersQueued && !state.SamplesPlayed )
      return mBuffer->getFormat().getSampleCount( mBuffer->getDuration() );
   else
