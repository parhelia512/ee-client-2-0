                ...
                "Populate the font cache for the specified font with characters from the specified string.")
{
   ...
   if(f == NULL)
   {
      Con::errorf("populateFontCacheString - could not load font '%s %d'", fontName.c_str(), fontSize);
      return;
   }

   if(!f->hasPlatformFont())
   {
      Con::errorf("populateFontCacheString - font '%s %d' has no platform font. Cannot generate more characters.", fontName.c_str(), fontSize);
      return;
   }
                ...
                "Note we only support BMP-0, so code points range from 0 to 65535.")
{
   ...
   if(f == NULL)
   {
      Con::errorf("populateFontCacheRange - could not load font '%s %d'", fontName.c_str(), fontSize);
      return;
   }
   ...
   if(!f->hasPlatformFont())
   {
      Con::errorf("populateFontCacheRange - font '%s %d' has no platform font Cannot generate more characters.", fontName.c_str(), fontSize);
      return;
   }
                ...
                "Populate the font cache for all fonts with characters from the specified string.")
{
   ...
   Con::printf("Populating font cache with string '%s'", argv[1]);
   ...
   while( theFont != NULL )
   {
      ...
      {
         // This has the side effect of generating character info, including the bitmaps.
         theFont->getStrWidthPrecise(argv[1]);
      }
      else
      {
         const String   fileName( theFont.getPath() );
                ...
                "exported as one long strip.")
{
   ...
   if(f == NULL)
   {
      Con::errorf("exportCachedFont - could not load font '%s %d'", fontName.c_str(), fontSize);
      return;
   }
                ...
                "same parameters you called exportCachedFont.")
{
   ...
   if(f == NULL)
   {
      Con::errorf("importCachedFont - could not load font '%s %d'", fontName.c_str(), fontSize);
      return;
   }
...
{
   const String   oldFontName(argv[1]);
   U32   fontSize = dAtoi(argv[2]);

   String newFontFile = GFont::getFontCacheFilename(argv[3], fontSize);

   // Load the original font.
   Resource<GFont> font = GFont::create(oldFontName, fontSize, Con::getVariable("$GUI::fontCacheDirectory"));

   // Deal with inexplicably missing or failed to load fonts.
   if (font == NULL)
   {
      Con::errorf(" o Couldn't find font : %s", oldFontName.c_str());
      return;
   }
