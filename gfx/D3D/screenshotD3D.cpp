...
void ScreenShotD3D::captureStandard()
{
   ...
   GFXD3DX.D3DXSaveSurfaceToFile( mFilename.utf16(), format, surface[1], NULL, NULL );
...
void saveRT_to_bitmap(GFXTexHandle &texToSave, const char *filename)
{
   ...
   GFXTexHandle sysTex;
   sysTex.set( size.x, size.y, GFXFormatR8G8B8X8, &GFXSystemMemProfile, avar("%s() - sysTex (line %d)", __FUNCTION__, __LINE__) );
