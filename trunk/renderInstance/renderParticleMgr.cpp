...
:  Parent( RenderParticleMgr::RIT_Particles, 1.0f, 1.0f, GFXFormatR8G8B8A8, 
...
RenderBinManager::AddInstResult RenderParticleMgr::addElement( RenderInst *inst )
{
   // Particles only
   ...
   if( pri->systemState != ParticleRenderInst::AwaitingOffscreenDraw )
      return RenderBinManager::arSkipped;

   // If offscreen rendering isn't enabled, set to high-res, and skip
   if(!mOffscreenRenderEnabled)
   {
      pri->systemState = ParticleRenderInst::AwaitingHighResDraw;
      return RenderBinManager::arSkipped;
   }
...
void RenderParticleMgr::render( SceneState *state )
{
      ...
      itr != mElementList.end(); itr++ )
   {
         ...
         itr2 != systemEntry.pInstances.end(); itr2++ )
      {
         ...
         // Sanity check
         if(ri->systemState == ParticleRenderInst::AwaitingOffscreenDraw)
         {
            ...
            if( !state->isDiffusePass() )
            {
               if(state->isReflectPass())
                  ...
                  ri->systemState = ParticleRenderInst::DrawComplete;
               continue;
            }
...
void RenderParticleMgr::renderInstance(ParticleRenderInst *ri, SceneState *state)
{
   // Draw system path, or draw composite path
   if(ri->systemState == ParticleRenderInst::DrawComplete)
      return;

   if(ri->systemState != ParticleRenderInst::AwaitingCompositeDraw)
   {
      // Set proper stateblock, and update state
      if(ri->systemState == ParticleRenderInst::AwaitingOffscreenDraw)
      {
         GFX->setStateBlock( _getOffscreenStateBlock(ri) );
         ri->systemState = ParticleRenderInst::AwaitingCompositeDraw;
         mParticleShaderConsts.mShaderConsts->set( mParticleShaderConsts.mModelViewProjSC, 
           *ri->modelViewProj * mOffscreenSystems[ri->targetIndex].clipMatrix );
      }
      else
      {
         if(ri->systemState == ParticleRenderInst::AwaitingMixedResDraw)
            GFX->setStateBlock( _getMixedResStateBlock( ri ) );
         else
            GFX->setStateBlock( _getHighResStateBlock( ri ) );
         ri->systemState = ParticleRenderInst::DrawComplete;
         mParticleShaderConsts.mShaderConsts->set( mParticleShaderConsts.mModelViewProjSC, *ri->modelViewProj );
      }
      ...
      mParticleShaderConsts.mShaderConsts->set( mParticleShaderConsts.mAlphaFactorSC, alphaFactor );
      mParticleShaderConsts.mShaderConsts->set( mParticleShaderConsts.mAlphaScaleSC, alphaScale );

      mParticleShaderConsts.mShaderConsts->set( mParticleShaderConsts.mFSModelViewProjSC, *ri->modelViewProj  );
      mParticleShaderConsts.mShaderConsts->set( mParticleShaderConsts.mOneOverFarSC, 1.0f / state->getFarPlane() );     

      if ( mParticleShaderConsts.mOneOverSoftnessSC )
      {
      ...
      GFXTextureObject *texObject = mPrepassTarget ? mPrepassTarget->getTargetTexture(0) : NULL;
      if( texObject )
      {
         GFX->setTexture( 1, texObject );

         Point4F rtParams;
         ScreenSpace::RenderTargetParameters(texObject->getSize(), mPrepassTarget->getTargetViewport(), rtParams);
   ...
   else if(ri->systemState == ParticleRenderInst::AwaitingCompositeDraw)
   {
      ...
      mParticleCompositeShaderConsts.mShaderConsts->set( mParticleCompositeShaderConsts.mScreenRect, *((Point4F *)&systemEntry.screenRect) );

      // Set offscreen texture
      Point4F rtParams;
      GFXTextureObject *particleSource = getTargetTexture(0, systemEntry.targetChainIdx);
      GFX->setTexture( 0, particleSource );
      if(particleSource)
      {
         ScreenSpace::RenderTargetParameters(particleSource->getSize(), getTargetViewport(), rtParams);
         mParticleCompositeShaderConsts.mShaderConsts->set( mParticleCompositeShaderConsts.mOffscreenTargetParamsSC, rtParams );
      }

      // And edges
      GFXTextureObject *texObject = mEdgeTarget ? mEdgeTarget->getTargetTexture(0) : NULL;
      GFX->setTexture( 1, texObject );
      if(texObject)
      {
         ScreenSpace::RenderTargetParameters(texObject->getSize(), mEdgeTarget->getTargetViewport(), rtParams);
         mParticleCompositeShaderConsts.mShaderConsts->set( mParticleCompositeShaderConsts.mEdgeTargetParamsSC, rtParams );
      }
         ...
         itr != systemEntry.pInstances.end(); itr++ )
      {
         ...
         if(pri->systemState == ParticleRenderInst::AwaitingCompositeDraw)
         {
            pri->systemState = ParticleRenderInst::AwaitingMixedResDraw;
...
bool RenderParticleMgr::_initShader()
{
   ...
   if ( mPrepassTarget )
      mPrepassTarget->getTargetShaderMacros( &macros );
   ...
   if ( mParticleShader && !mParticleShaderConsts.mShaderConsts )
   {
   ...
   if ( mParticleCompositeShader && !mParticleCompositeShaderConsts.mShaderConsts )
   {
...
{
   RenderPassManager *rpm = getParentManager();
   if ( !rpm )
      return;
   ...
   mPrepassTarget = MatTextureTarget::findTargetByName( "prepass" );
   mEdgeTarget = MatTextureTarget::findTargetByName( "edge" );
