...
bool Win32FileSystemChangeNotifier::internalRemoveNotification( const Path &dir )
{
   for ( U32 i = 0; i < mDirs.size(); ++i )
   {
      if ( mDirs[i] == dir )
      {
//          Con::printf( "[Win32FileSystemChangeNotifier::internalRemoveNotification] : [%s]", dir.getFullPath().c_str() );
...
{
   U32 numHandles = mHandleList.size();
   if ( numHandles == 0 )
      return;

   DWORD dwWaitStatus = WaitForMultipleObjects(numHandles, mHandleList.address(), FALSE, 0);
...
U32 Win32File::calculateChecksum()
{
   ...
   U32 bytesRead = read(buf, fileSize);
   ...
   return CRC::calculateCRC(buf, fileSize);
}
