...
void blTerrainProxy::lightVector(LightInfo * light)
{
   ...
   for (U32 y = 0; y < mLightMapSize; y++)
   {
      for (U32 x = 0; x < mLightMapSize; x++)
      {
         ...
         RayInfo info;
         if(terrain->getContainer()->castRay(pixelPos3F, lightPos, STATIC_COLLISION_MASK, &info))
         {
            // Shadow the pixel.
            shadowed = 1.0f;
         }
...
// TODO: Review this... the BasicLightManager should probably
// be the one to create this thing!
