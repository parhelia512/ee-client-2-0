...
{
   String uname(name);
   if (shape->findName(uname) != -1)
   {
      // Try appending A-Z to the name to make it unique
      for (char suffix = 'A'; suffix <= 'Z'; suffix++)
      {
         if (shape->findName(uname + suffix) == -1)
         {
            uname += suffix;
            break;
         }
...
void TSShapeLoader::recurseSubshape(AppNode* appNode, S32 parentIndex, bool recurseChildren)
{
   ...
   else
   {
      ...
      shape->nodes.last().nameIndex = shape->addName(getUniqueName(appNode->getName()));
      ...
      if (appNode->isBillboard())
      {
         ...
         if (!dStrEqual(dname, appNode->getName()))
         {
            ...
            shape->addBillboardDetail( dname,
                                       size,
                                       numEquatorSteps,
                                       numPolarSteps,
                                       dl,
...
void TSShapeLoader::generateObjects()
{
   for (S32 iSub = 0; iSub < subshapes.size(); iSub++)
   {
      ...
      for (int iMesh = 0; iMesh < subshape->objMeshes.size(); iMesh++)
      {
         ...
         mesh->detailSize = 2;
         meshNames.push_back( String::GetTrailingNumber( mesh->getName(), mesh->detailSize) );
...
void TSShapeLoader::generateMaterialList()
{
   ...
   for (int iMat = 0; iMat < AppMesh::appMaterials.size(); iMat++)
   {
      ...
      if (appMat->getFlags() & TSMaterialList::IflMaterial)
      {
         ...
         shape->iflMaterials.last().nameIndex = shape->addName(appMat->getName());
      }
   }
}
...
void TSShapeLoader::install()
{
   ...
   {
      shape->bounds.minExtents += shapeOffset;
      shape->bounds.maxExtents += shapeOffset;
   }
   else
