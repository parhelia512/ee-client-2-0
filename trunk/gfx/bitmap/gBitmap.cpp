...
Resource<GBitmap> GBitmap::load(const Torque::Path &path)
{
   ...
   const String   origExtension( String::ToLower( path.getExtension() ) );

   Resource<GBitmap> ret;
   Vector<Path>      foundPaths( __FILE__, __LINE__ );

   bool  found = GBitmap::sFindFiles( path, &foundPaths );

   if ( found )
   {
      for( U32 i = 0; i < foundPaths.size(); i++ ) 
      {
         if ( foundPaths[i].getExtension() == origExtension )  // we've already tried this one
            continue;

         ret = ResourceManager::get().load( foundPaths[i] );
...
ConsoleFunction( getBitmapInfo, const char *, 2, 2, "( filename )\n"
                "Returns image info in the following format: \n"
                "<width>\t<height>\t<bytes per pixel>" )
{
   // Pull this out.
   const UTF8 *fileName = argv[1];
   ...
   dSprintf( returnBuffer, 256, "%d\t%d\t%d",   image->getWidth(), 
                                                image->getHeight(),
