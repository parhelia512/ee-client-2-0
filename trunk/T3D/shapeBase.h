...
protected:
   ...
   bool _prepRenderImage(  SceneState* state, 
                           const U32 stateKey, 
                           const U32 startZone, 
   ...
   void _renderBoundingBox( ObjectRenderInst *ri, SceneState *state, BaseMatInstance* );
...
public:
   ...
   enum ShapeBaseMasks {
      ...
      HideCloakMask   = Parent::NextFreeMask << 4,       ///< Hiding/cloaking state changed.
...
public:
   ...
   virtual void renderMountedImage( U32 imageSlot, TSRenderState &state );
   /// @}
   ...
   /// Casts a ray from start to end testing against rendered geometry.
   /// Stores gathered information in 'info' returns true if successful.
   /// @param   start   Start point for ray
   /// @param   end     End point for ray
   /// @param   info    Information from raycast (out)
