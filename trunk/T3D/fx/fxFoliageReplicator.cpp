      ...
      do
      {
         ...
         if (CollisionResult)
         {
            ...
            if (((CollisionType & TerrainObjectType) && !mFieldData.mAllowOnTerrain)	||
               ...
               ((CollisionType & ( StaticTSObjectType | StaticShapeObjectType )) && !mFieldData.mAllowStatics)	||
...
void fxFoliageReplicator::SetupShader()
{
   ...
   mShaderData->mapSamplerNames( mFoliageShaderConsts );
                                          ...
                                          const bool /*modifyBaseZoneState*/)
{
   ...
   // Is Object Rendered?
   if ( !state->isShadowPass() && state->isObjectRendered(this))
   {
...
void fxFoliageReplicator::renderObject(ObjectRenderInst *ri, SceneState *state, BaseMatInstance* overrideMat)
{
   if (overrideMat)
      return;
   ...
   if (mCurrentFoliageCount > 0) {
      ...
      if (!mFieldData.mHideFoliage) {
         ...
         {
            if (mFoliageShaderGlobalSwayPhaseSC->isValid())
               mFoliageShaderConsts->set(mFoliageShaderGlobalSwayPhaseSC, mGlobalSwayPhase);

            if (mFoliageShaderSwayMagnitudeSideSC->isValid())
               mFoliageShaderConsts->set(mFoliageShaderSwayMagnitudeSideSC, mFieldData.mSwayMagnitudeSide);

            if (mFoliageShaderSwayMagnitudeFrontSC->isValid())
               mFoliageShaderConsts->set(mFoliageShaderSwayMagnitudeFrontSC, mFieldData.mSwayMagnitudeFront);

            if (mFoliageShaderGlobalLightPhaseSC->isValid())
               mFoliageShaderConsts->set(mFoliageShaderGlobalLightPhaseSC, mGlobalLightPhase);

            if (mFoliageShaderLuminanceMagnitudeSC->isValid())
               mFoliageShaderConsts->set(mFoliageShaderLuminanceMagnitudeSC, LuminanceMagnitude);

            if (mFoliageShaderLuminanceMidpointSC->isValid())
               mFoliageShaderConsts->set(mFoliageShaderLuminanceMidpointSC, LuminanceMidPoint);
            ...
            if (mFoliageShaderProjectionSC->isValid())
            ...
            if (mFoliageShaderWorldSC->isValid())
               mFoliageShaderConsts->set(mFoliageShaderWorldSC, world);

            Point3F camPos = state->getCameraPosition();

            if (mFoliageShaderDistanceRangeSC->isValid())
               mFoliageShaderConsts->set(mFoliageShaderDistanceRangeSC, MaximumViewDistance);

            if (mFoliageShaderCameraPosSC->isValid())
               mFoliageShaderConsts->set(mFoliageShaderCameraPosSC, camPos);

            if (mFoliageShaderTrueBillboardSC->isValid())
               mFoliageShaderConsts->set(mFoliageShaderTrueBillboardSC, mFieldData.mUseTrueBillboards ? 1.0f : 0.0f );

            if (mFoliageShaderGroundAlphaSC->isValid())
               mFoliageShaderConsts->set(mFoliageShaderGroundAlphaSC, Point4F(mFieldData.mGroundAlpha, mFieldData.mGroundAlpha, mFieldData.mGroundAlpha, mFieldData.mGroundAlpha));

            if (mFoliageShaderAmbientColorSC->isValid())
            {
               const ColorF &ambient = state->getLightManager()->getSpecialLight( LightManager::slSunLightType )->getAmbient();
