...
static S32 sRenderPlayerCollision = false;
      ...
      for( i=0; i<PlayerData::NUM_SPLASH_EMITTERS; i++ )
      {
         if ( mDataBlock->splashEmitterList[i] ) 
         {
            ...
            if( !mSplashEmitter[i]->registerObject() )
            {
               ...
               delete mSplashEmitter[i].getObject();
...
{  
   ...
   // NOTE: we aren't testing for overlaps if there is no physics player!
   // Do standard Torque collision tests here!!
   if ( !mPhysicsPlayer )
      return true;
...
void Player::renderMountedImage( U32 imageSlot, TSRenderState &rstate )
{
   GFX->pushWorldMatrix();
   ...
   if ( !rstate.getSceneState()->isShadowPass() && isFirstPerson() && data.useEyeOffset ) 
   {
      ...
      {
         //MatrixF mat;
         //Parent::getRenderImageTransform( imageSlot, &mat);
         ...
         MatrixF offsetMat( data.eyeOffset );
