...
// If we're locking the mouse signaling that we've gained focus will lock the mouse.  If we need to restore fullscreen
// that will also happen on GainFocus.  By suppressing the signal unless we are the only visible window we can open the console
// and not worry that moving/resizing the main window or making it key will take away our mouse or drop us back into full screen.
// When the console is closed, we will actually signal that we've gained focus and do whatever is appropriate.
//
// Of course, this is mostly personal preference.
...
{
   // we cache this value before we send the raw key events so that the effects of the key events don't affect whether we send char events.
   bool wantsCharEvents = mTorqueWindow->getKeyboardTranslation();
   
   [self rawKeyUpDown:theEvent keyDown:YES];
   
   // did torque decide it doesn't want char events after the above raw key event?
   wantsCharEvents &= mTorqueWindow->getKeyboardTranslation();
