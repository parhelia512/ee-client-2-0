...
void ImposterCapture::_separateAlpha( GBitmap *imposterOut )
{
   PROFILE_START(TSShapeInstance_snapshot_sb_separate);
      ...
      // We use a random later in the image processing
      // to generate some "fizzle" in the alpha channel.
      const U32 pixCount = mDim * mDim;
      MRandom rand( pixCount );
      ...
      for ( U32 i=0; i < pixCount; i++ )
      {
         ...
         if ( alpha != 0 )
         {
            ...
            // to disolve intead of popping into view.
            alpha /= 3;
            if ( alpha > 84 )
               alpha = rand.randI( 85, 255 );
            dst[i*4+3] = (U8)alpha;
         }
         else
         {
   ...
   PROFILE_START(TSShapeInstance_snapshot_sb_filter);
      ...
      // in essience give us a border around the edges of the 
      // imposter sillhoute which fixes the artifacts around the
                              ...
                              const Point3F &center )
{
   ...
   GFX->setViewport( RectI( 0, 0, mDim, mDim ) );
                                 ...
                                 GBitmap **normalMapOut )
{
   ...
   (*normalMapOut) = new GBitmap( mDim, mDim, false, GFXFormatR8G8B8 );

   // Move the object down the x axis and apply its center offset.
   MatrixF offset( EulerF( 0, 0, 0 ), Point3F( -10.0f * mRadius, 0, 0 ) );
   ...
   MatrixF objMatrix;
   objMatrix.mul( offset, centerMat );
   GFX->setWorldMatrix( objMatrix );

   gClientSceneGraph->pushRenderPass( mRenderPass );
   ...
   _renderToTexture( mNormalTex, (*normalMapOut), ColorI(128, 128, 128) );
