...
protected:
   ...
   /// This is a old hack to fix a terrain performance
   /// issue when first seeing a terrain from inside
   /// of an interior.
   bool mHasRendered;
   ...
   void _renderBlock( SceneState *state );
   void _renderDebug( ObjectRenderInst *ri, SceneState *state, BaseMatInstance *overrideMat );
   ...
   // Protected fields
   static bool _setTerrainFile( void *object, const char *data );
   static bool _setSquareSize( void *object, const char *data );
   static bool _setBaseTexSize( void *object, const char *data );
...
public:
   ...
   /// material index for a 2d position in the terrains object
   /// space.
   ///
   /// If the terrain at that point is within an empty block
   /// or the 2d position is outside of the terrain area then
...
public:
   ...
   void setFile( const FileName& terrFileName );
   ...
   bool isTiling() const { return mTile; }
