...
OnTorqueStartup
{
   FEATUREMGR->registerFeature( MFT_TerrainEmpty, new TerrainEmptyFeatHLSL );   
...
Var* TerrainFeatHLSL::_getUniformVar( const char *name, const char *type )
{
...
Var* TerrainFeatHLSL::_getDetailIdStrengthParallax()
{
   ...
   if ( !detailInfo )
   {
      ...
      detailInfo->constSortPos = cspPass;
   }
                                          ...
                                          const MaterialFeatureData &fd )
{
   ...
   Var *inTex;
   {
      ...
      Var *oneOverTerrainSize = _getUniformVar( "oneOverTerrainSize", "float" );
   ...
   Var *squareSize = _getUniformVar( "squareSize", "float" );
...
void TerrainEmptyFeatHLSL::processVert(   Vector<ShaderComponent*> &componentList,
                                          const MaterialFeatureData &fd )
{
   ShaderConnector *connectComp = dynamic_cast<ShaderConnector *>( componentList[C_CONNECTOR] );

   Var *inEmpty = getVertTexCoord( "tcEmpty" );

   // Pass the empty state down to the pixel shader too.
   Var *outEmpty = connectComp->getElement( RT_TEXCOORD );
   outEmpty->setName( "outEmpty" );
   outEmpty->setStructName( "OUT" );
   outEmpty->setType( "float" );
   outEmpty->mapsToSampler = false;
   output = new GenOp( "   @ = @;\r\n", outEmpty, inEmpty );
}

void TerrainEmptyFeatHLSL::processPix( Vector<ShaderComponent*> &componentList, 
                                       const MaterialFeatureData &fd )
{
   // Now we clip empty terrain squares.
   //
   // We packed a 1 or -1 into the empty texcoord based
   // on if the vertex was part of an empty square or not.
   //
   // Since the values from the pixel shader are
   // interplolated we add a value close to 1 in
   // order to limit the clipping to triangles 
   // where all 3 verts are -1.
   //
   Var *inEmpty = getInTexCoord( "tcEmpty", "float", false, componentList );
   output = new GenOp( "   clip( @ + 0.999 );\r\n", inEmpty );
}

ShaderFeature::Resources TerrainEmptyFeatHLSL::getResources( const MaterialFeatureData &fd )
{
   Resources res; 
   res.numTexReg = 1;
   return res;
                                             ...
                                             const MaterialFeatureData &fd )
{
   ...
   detScaleAndFade->constSortPos = cspPass;
                                             ...
                                             const MaterialFeatureData &fd )
{
   ...
   //
   // TODO: I disabled branching as it currently causes 
   // sparkling artifacts... this is probably an issue of
   // not using tex2Dlod() or that i'm using ansiotropic
   // fliltering... maybe?
   //
   if ( false && GFX->getPixelShaderVersion() >= 3.0f )
