...
@implementation OverwriteConfirmation

- ( NSString* ) panel: ( id) sender userEnteredFilename: ( NSString* ) filename confirmed: ( BOOL ) okFlag
{
   // We want the real filename with the absolute path *and* the extension.
   const char* file = [ [ sender filename ] UTF8String ];
   
   if( okFlag && Platform::isFile( file ) )
   {
      String message = String::ToString( "Do you really want to overwrite '%s'.", file );
      if( !Platform::AlertOKCancel( "Confirm overwrite", message ) )
         return nil;
   }
...
static NSArray* _MacCocoaCreateAndRunSavePanel(FileDialogData &mData)
{
   ...
   OverwriteConfirmation* delegate = nil;
   if( mData.mStyle & FileDialogData::FDS_OVERWRITEPROMPT )
   {
      delegate = [ OverwriteConfirmation new ];
