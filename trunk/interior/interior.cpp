                          ...
                          const bool     flipClipPlanes)
{
   ...
   if(modifyBaseState || dontRestrictOutside )
   {
      ...
      sgStoredFrustum[0] = state->getBaseZoneState().frustum.getNearLeft();
      sgStoredFrustum[1] = state->getBaseZoneState().frustum.getNearRight();
      sgStoredFrustum[2] = state->getBaseZoneState().frustum.getNearBottom();
      sgStoredFrustum[3] = state->getBaseZoneState().frustum.getNearTop();
   ...
   else
   {
      ...
      sgStoredFrustum[0] = state->getZoneState(containingZone).frustum.getNearLeft();
      sgStoredFrustum[1] = state->getZoneState(containingZone).frustum.getNearRight();
      sgStoredFrustum[2] = state->getZoneState(containingZone).frustum.getNearBottom();
      sgStoredFrustum[3] = state->getZoneState(containingZone).frustum.getNearTop();
      sgStoredFrustum[4] = state->getNearPlane();
      sgStoredFrustum[5] = state->getFarPlane();
   }
   ...
   for(U32 i = 1; i < mZones.size(); i++)
   {
      ...
      if(rState.render)
      {
         rState.frustum.set(  rState.frustum.isOrtho(),
                              ...
                              rState.frustum.getNearDist(), 
                              rState.frustum.getFarDist() );
                         ...
                         Vector<PlaneRange>& planeRangeStack)
{
   ...
   for(S32 i = rZone.portalStart; i < U32(rZone.portalStart + rZone.portalCount); i++)
   {
      ...
      if(planeIsFlipped(rPortal.planeIndex))
         camSide = -camSide;
      S32  zoneSide = (U32(rPortal.zoneFront) == currZone) ? PlaneF::Front : PlaneF::Back;
      U16 otherZone = (U32(rPortal.zoneFront) == currZone) ? rPortal.zoneBack : rPortal.zoneFront;
