...
void GuiRoadEditorCtrl::initPersistFields()
{
   ...
   //addField( "MoveNodeCursor", TypeSimObjectPtr, Offset( mMoveNodeCursor, GuiRoadEditorCtrl) );
   //addField( "AddNodeCursor", TypeSimObjectPtr, Offset( mAddNodeCursor, GuiRoadEditorCtrl) );
   //addField( "InsertNodeCursor", TypeSimObjectPtr, Offset( mInsertNodeCursor, GuiRoadEditorCtrl) );
   //addField( "ResizeNodeCursor", TypeSimObjectPtr, Offset( mResizeNodeCursor, GuiRoadEditorCtrl) );
...
void GuiRoadEditorCtrl::on3DMouseDown(const Gui3DMouseEvent & event)
{
   ...
   else if ( mMode == mAddRoadMode )
   {
		if ( nodeClicked && clickedRoadPtr )
      {
		...
		Material * defaultMat = dynamic_cast<Material*>( Sim::findObject("DefaultDecalRoadMaterial") );
		if(  defaultMat )
				newRoad->mMaterialName = "DefaultDecalRoadMaterial";
...
void GuiRoadEditorCtrl::renderScene(const RectI & updateRect)
{
   ...
   if ( mSelRoad )
   {
      _drawRoadSpline( mSelRoad, mSelectedSplineColor );     
   }
...
void GuiRoadEditorCtrl::setSelectedRoad( DecalRoad *road )
{
   ...
   if ( road != NULL )
      Con::executef( this, "onRoadSelected", road->scriptThis() );
   else
