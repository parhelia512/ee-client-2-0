...
namespace {
   ...
   static EnumTable::Enums gAlignmentEnums[] = {
      { World, "World" },
      { Object, "Object" }      
   };

   EnumTable gAlignmentEnumTable( AlignEnumCount, gAlignmentEnums );

   static EnumTable::Enums gModeEnums[] = {
      { NoneMode, "None" },
      { MoveMode, "Move" },      
      { RotateMode, "Rotate" },
      { ScaleMode, "Scale" }
   ...
   EnumTable gModeEnumTable( ModeEnumCount, gModeEnums );
...
{
   addField( "alignment",           TypeEnum,   Offset(alignment, GizmoProfile ), 1, &gAlignmentEnumTable );
   addField( "mode",                TypeEnum,   Offset(mode, GizmoProfile ), 1, &gModeEnumTable );
...
Point3F Gizmo::getScale() const
{
   //Point3F scale = mScale;
   //mToObjectMat.mulV(scale);
   //return scale;
   return mScale;
}
...
{   
   ...
   mSelectionIdx = -1;
   Point3F end = mCameraPos + event.vec * smProjectDistance;
   ...
   if ( ( ( mProfile->mode == MoveMode ) && ( mProfile->flags & GizmoProfile::CanTranslateUniform ) ) ||
        ( ( mProfile->mode == ScaleMode ) && ( mProfile->flags & GizmoProfile::CanScaleUniform ) ) )
   {
      ...
      if ( uhandle.collideLine( mCameraPos, end ) )
      {
         mSelectionIdx = 6;
         return true;
      }
        ...
        ( mProfile->flags & GizmoProfile::PlanarHandlesOn ) )
   {
      for ( U32 i = 0; i < 3; i++ )
      {
         ...
         Point3F poly [] = {
            Point3F(mOrigin + p1 + p2 * 0.75f),
            Point3F(mOrigin + p1 + p2),
            Point3F(mOrigin + p1 * 0.75f + p2),
            Point3F(mOrigin + (p1 + p2) * 0.75f)
         };

         Point3F end = mCameraPos + event.vec * smProjectDistance;
         F32 t = plane.intersect(mCameraPos, end);
         if ( t >= 0 && t <= 1 )
         {
            Point3F pos;
            pos.interpolate(mCameraPos, end, t);
...
void Gizmo::on3DMouseDown( const Gui3DMouseEvent & event )
{
   ...
   if ( mProfile->mode == MoveMode || mProfile->mode == ScaleMode )
   {
      ...
      else if ( mSelectionIdx >= 3 && mSelectionIdx <= 5 )
      {
...
{   
   ...
   {      
      ...
      if ( mProfile->mode == MoveMode )
      {
         ...
         VectorF totalOffset = projPnt - mMouseDownProjPnt;         
         Point3F newPosition = mSavedTransform.getPosition() + totalOffset ;         
   ...
   else if ( mProfile->mode == RotateMode )
   {
      ...
      bool doScreenRot = ( mSelectionIdx == 6 );
      ...
      F32 offsetDistWS = ( offsetDist * ( mOrigin - mCameraPos ).len() ) / mLastWorldToScreenScale.y;

      // The angle that we will rotate the (saved) gizmo transform by to 
      // generate the current gizmo transform.
      F32 angle = offsetDistWS * mSign * mProfile->rotateScalar;
      ...
      EulerF totalRot(0,0,0);
      totalRot[rotAxisIdx] = angle;    

      if ( doScreenRot )
      {
         // Transform the rotation around the y axis in a camera-relative
         // space into world space.
         
         VectorF fvec( mOrigin - mCameraPos );
         fvec.normalizeSafe();
         VectorF rvec;
         mCameraMat.getColumn( 0, &rvec );
         VectorF uvec = mCross( rvec, fvec );
         uvec.normalizeSafe();
         rvec = mCross( fvec, uvec );
         rvec.normalizeSafe();

         MatrixF xfm( true );
         xfm.setColumn( 0, rvec );
         xfm.setColumn( 1, fvec );
         xfm.setColumn( 2, uvec );
         
         xfm.mulV(totalRot);
         xfm.mulV(mDeltaRot);

         // Transform the world space rotation into gizmo space.

         MatrixF xfm2( mSavedTransform );
         xfm2.inverse();
...
void Gizmo::renderGizmo(const MatrixF &cameraTransform)
{
   ...
   GFX->setFrustum( l, r, b, t, n, 100000.0f );
   ...
   if ( mProfile->mode == RotateMode )
   {
      ...
      for(U32 i = 0; i < 3; i++)
      {
         PrimBuild::color( (i == mSelectionIdx) ? mProfile->axisColors[i] : mProfile->inActiveColor );
   ...
   if ( mUniformHandleEnabled && ( mProfile->mode == MoveMode || mProfile->mode == ScaleMode ) )
   {            
      if ( mSelectionIdx >= 3 && mSelectionIdx <= 5 )
      {         
         U32 idx0 = sgPlanarVectors[mSelectionIdx-3][0];
         U32 idx1 = sgPlanarVectors[mSelectionIdx-3][1];

         Point3F& p0 = midpnt[idx0];
         Point3F& p1 = midpnt[idx1];       

         PrimBuild::color4i( 237, 219, 0, 150 );
...
{   
   ...
   Point3F dir = mOrigin - mCameraPos;
	...
	mProjLen = ( (F32)mProfile->screenLen * dist ) / mLastWorldToScreenScale.y;
}
...
void Gizmo::_renderAxisCircles()
{
   ...
   // Render the ScreenSpace rotation circle...
   {
      ...
      PrimBuild::color( (mSelectionIdx == 6) ? mProfile->activeColor : mProfile->inActiveColor );
...
void Gizmo::_renderPlane()
{
   ...
   if ( mProfile->renderPlane )
      GFX->getDrawUtil()->drawSolidPlane( desc, mOrigin, size, mProfile->gridColor );
   ...
   if ( mProfile->renderPlaneHashes )
   {
      ...
      GFX->getDrawUtil()->drawPlaneGrid( desc, mOrigin, size, Point2F( mProfile->gridSize.x, mProfile->gridSize.y ), gridColor );
   }
}
...
Align Gizmo::_filteredAlignment()
{
   Align align = mProfile->alignment;
...
{   
   ...
   Align desired = _filteredAlignment();
