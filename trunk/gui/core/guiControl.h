...
private:
   ...
   String                  mScriptFile; ///< The script file this GUI was created from
...
public:
   ...
   enum
   {
      IgnoreCanSave        = BIT( 30 ),   ///< Write out even if mCanSave=false.
   ...
   static bool setTooltipProfileProt( void *obj, const char *data );
   ...
   bool defaultTooltipRender( const Point2I &hoverPos, const Point2I &cursorPos, const char* tipText = NULL );
	...
	typedef Vector< Vector< GuiWindowCollapseCtrl *> >	CollapseGroupVec;
...
public:
    ...
    /// Associates the gui with a script file.
    void setScriptFile( const String& filename );
...
public:
    ...
    void clearFirstResponder(bool useCallback=false);
