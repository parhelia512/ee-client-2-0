...
S32 GuiAviBitmapCtrl::vcmDraw(U64 dwICflags)
{
   ...
   for (U32 j = 0; j < mHeightCount; j++)
   {
      ...
      for (U32 i = 0; i < mWidthCount; i++)
      {
         ...
         {
            const U8 *src = &mPBuf[bmp->getBytesPerPixel()*((mBitmapHeight-(y+lp+1))*mBitmapAlignedWidth + x)];
            U8 *dest = bmp->getAddress(0, lp);

            // counting on the artist to switch the R & B channels so we don't have to in runtime
            if (!mSwapRB)
               dMemcpy(dest, src, width * bmp->getBytesPerPixel());
            else
               Swizzles::bgr.ToBuffer( dest, src, width * bmp->getBytesPerPixel() );
         }
   ...
   if (mTextureHandles)
   {
      ...
      for (U32 j = 0; j < mHeightCount; j++)
      {
         ...
         for (U32 i = 0; i < mWidthCount; i++)
         {
            ...
            for (U32 lp = 0; lp < height; lp++)
            {
               const U8 *src = &mPBuf[bmp->getBytesPerPixel()*((y+lp)*mBitmapAlignedWidth + x)];
               U8 *dest = bmp->getAddress(0, lp);
