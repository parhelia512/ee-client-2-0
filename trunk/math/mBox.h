...
class Box3F
{
  ...
  public:
   ...
   F32 len(S32 axis) const { return ((F32*)&maxExtents.x)[axis] - ((F32*)&minExtents.x)[axis]; }
...
inline Point3F Box3F::getExtents() const
{
...
inline F32 Box3F::getSqDistanceToPoint( const Point3F &refPt ) const
{
   ...
   for ( U32 i=0; i < 3; i++ )
   {
      const F32 v = refPt[i];
      if ( v < minExtents[i] )
         sqDist += ( minExtents[i] - v ) * ( minExtents[i] - v );
      if ( v > maxExtents[i] )
         sqDist += ( v - maxExtents[i] ) * ( v - maxExtents[i] );
   }
