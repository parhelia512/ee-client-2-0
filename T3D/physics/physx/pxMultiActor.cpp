...
IMPLEMENT_CONSOLETYPE(PxMultiActorData)
IMPLEMENT_GETDATATYPE(PxMultiActorData)
IMPLEMENT_SETDATATYPE(PxMultiActorData)
...
void PxMultiActorData::initPersistFields()
{
   ...
   addGroup( "Physics" );
      ...
      addField( "material", TypePxMaterialPtr, Offset( material, PxMultiActorData ) );
      ...
      for ( S32 i=0; i < MaxCorrectionNodes; i++ )
      {
         ...
         addField( buff, TypeString, Offset( correctionNodeNames[i], PxMultiActorData ) );
      }
...
void PxMultiActor::initPersistFields()
{
   ...
   */
   ...
   addGroup("Lighting");
      addField("receiveSunLight", TypeBool, Offset(receiveSunLight, PxMultiActor));
      addField("receiveLMLighting", TypeBool, Offset(receiveLMLighting, PxMultiActor));
      //addField("useAdaptiveSelfIllumination", TypeBool, Offset(useAdaptiveSelfIllumination, TSStatic));
      addField("useCustomAmbientLighting", TypeBool, Offset(useCustomAmbientLighting, PxMultiActor));
      //addField("customAmbientSelfIllumination", TypeBool, Offset(customAmbientForSelfIllumination, TSStatic));
      addField("customAmbientLighting", TypeColorF, Offset(customAmbientLighting, PxMultiActor));
      addField("lightGroupName", TypeRealString, Offset(lightGroupName, PxMultiActor));
   endGroup("Lighting");

   addGroup( "Debug" );
      addField( "debugRender", TypeBool, Offset( mDebugRender, PxMultiActor ) );
      addField( "broken", TypeBool, Offset( mBroken, PxMultiActor ) );
   endGroup( "Debug" );
...
bool PxMultiActor::onAdd()
{
   ...
   mIsDummy = isClientObject() && gPhysicsPlugin->isSinglePlayer(); //&& mDataBlock->singlePlayerOnly;
...
void PxMultiActor::_destroyActors()
{
   ...
   for( S32 i = 0; i < mJoints.size(); i++ )
   {
      ...
      PxJointUserData *jointData = PxJointUserData::getData( *joint );
      if ( jointData )
         delete jointData;
...
PxJointUserData* PxMultiActor::_createJointUserData( NxJoint *joint, String &userProperties )
{
   PxJointUserData *jointData = new PxJointUserData();
...
bool PxMultiActor::onNewDataBlock(GameBaseData* dptr)
{
...
bool PxMultiActor::prepRenderImage( SceneState *state,
                                       const U32 stateKey,
                                       const U32 startZone,
...
void PxMultiActor::_debugRender( ObjectRenderInst *ri, SceneState *state, BaseMatInstance *overrideMat )
{
   if ( mShapeInstance )
   {
      GFXTransformSaver saver;
...
void PxMultiActor::_onJointBreak( NxReal breakForce, NxJoint &brokenJoint )
{
   ...
   PxJointUserData *jointData = PxJointUserData::getData( brokenJoint );
   ...
   // Server objects don't do the following client side effects.
   // ... unless it's single player!
   if ( isServerObject() && !gPhysicsPlugin->isSinglePlayer() )
      return;

   // JCFHACK: If this is a server object trying to play a client side
   // effect in a single player situation, it could happen before the client
   // is actually initialized and connected!
   SimObject *temp = Sim::findObject( "ClientMissionCleanup");
   if ( !temp )
      return;

   ParticleEmitterData *particleData = NULL;
   if ( !Sim::findObject( "JointBreakEmitter", particleData ) )
      return;

   ParticleEmitter * emitter = new ParticleEmitter;
   emitter->onNewDataBlock( particleData );

   MatrixF jointMat( true );
   jointMat.setPosition( jointPos );
   ...
   {
      emitter->emitParticles( jointPos, Point3F( 0.0, 0.0, 1.0 ), 0.25f, Point3F( 0, 0, 0 ), 10 );
      emitter->deleteWhenEmpty();
   }
}

void PxMultiActor::_onContact(   NxActor *ourActor,
                                 NxActor *hitActor,
                                 SceneObject *hitObject,
                                 const Point3F &hitPoint,
                                 const Point3F &hitForce )
{
   PxUserData *data = PxUserData::getData( *ourActor );
   if (  data &&
         !data->mIsBroken &&
...
void PxMultiActor::mountObject( SceneObject *obj, U32 node )
{
   ...
   obj->mMount.node = (node >= 0 && node < PxMultiActorData::NumMountPoints)? node: 0;
                              ...
                              bool isBroken )
{
   ...
   else if ( isServerObject() && gPhysicsPlugin->isSinglePlayer() )
   {
