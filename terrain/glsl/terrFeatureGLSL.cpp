...
OnTorqueStartup
{
   FEATUREMGR->registerFeature( MFT_TerrainEmpty, new TerrainEmptyFeatGLSL );   
...
Var* TerrainFeatGLSL::_getUniformVar( const char *name, const char *type )
{
...
void TerrainEmptyFeatGLSL::processVert(   Vector<ShaderComponent*> &componentList,
                                       const MaterialFeatureData &fd )
{
   ShaderConnector *connectComp = dynamic_cast<ShaderConnector *>( componentList[C_CONNECTOR] );
...
void TerrainEmptyFeatGLSL::processPix( Vector<ShaderComponent*> &componentList, 
                                      const MaterialFeatureData &fd )
{
   ...
   //
   Var *inEmpty = getInTexCoord( "outEmpty", "float", false, componentList );
   output = new GenOp( "   if (@ + 0.999 < 0.0)\r\n discard;\r\n", inEmpty );
}

ShaderFeature::Resources TerrainEmptyFeatGLSL::getResources( const MaterialFeatureData &fd )
{
   Resources res; 
   res.numTexReg = 1;
   return res;
