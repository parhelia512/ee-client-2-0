                           ...
                           const Vector<String> &materials )
{
   ...
   dSprintf( missionName, sizeof(missionName), "%s\0", inOutFilename->c_str() );
   ...
   for (U32 i = 0; i < 32; i++)
   {
      ...
      if (dStrlen(missionName) == 0 || dStricmp(missionName, "0") == 0)
         dSprintf(testName, sizeof(testName), "levels/terrain_%d.ter\0", i );
      else
         dSprintf( testName, sizeof(testName), "%s_%d.ter\0", missionName, i );
